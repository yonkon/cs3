...
<div id="account_info_{$block.snapping_id}">
	...
	{hook name="profiles:my_account_menu"}
		{if $auth.user_id}
			...
			{else}
				...
				{/if}
			{/if}
            {if $user_info.user_type == 'P'}
                <li><a href="{'agents.office'|fn_url}" rel="nofollow" class="underlined">{$lang.agents_office}</a></li>
            {/if}
	...
	<div class="buttons-container">
		{if $auth.user_id}
            ...
            <a href="{"agents.add_subagent"|fn_url}" rel="nofollow" class="account">{$lang.register}</a>
		{else}
