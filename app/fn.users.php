...
function fn_delete_user($user_id)
{
    ...
    if (!fn_check_rights_delete_user($user_data, $auth)) {
        fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('user_cannot_be_deleted', array(
            '[user_id]' => $user_id
    ...
    if ($ekey) {
        ...
        if ($u_id) {
            ...
            if ($user_status == LOGIN_STATUS_OK) {
                fn_set_notification('N', fn_get_lang_var('notice'), fn_get_lang_var('text_change_password'), 'I', 'notice_text_change_password');
