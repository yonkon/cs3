...
if ($_SERVER['REQUEST_METHOD']	== 'POST') {
	...
	if ($mode == 'delete') {
		...
		if (!empty($_REQUEST['plan_id'])) {
			...
			} elseif (!empty($_REQUEST['product_ids'])) {
			...
			} elseif (!empty($_REQUEST['promotion_ids'])) {
