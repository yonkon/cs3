...
function fn_get_aff_banner_html($type, $banner_data, $mode = '', $partner_id = '', $lang_code = CART_LANGUAGE)
{
	...
	if ($banner_data['type'] == 'P') {
		...
		if (!empty($product_id)) {
			...
			$prod_data = fn_get_product_data($product_id, $auth, CART_LANGUAGE, "?:products.product_id, ?:products.company_id, product, full_description as product_full_description, short_description as product_short_description", false, true, false);
...
function fn_affiliate_get_predefined_statuses($type, &$statuses)
{
	if ($type == 'affiliate') {
		$statuses['affiliate'] = array(
			'O' => fn_get_lang_var('open'),
			'S' => fn_get_lang_var('successful')
		);
	}
}
...
function fn_affiliate_check_user_type_access_rules($user_data, &$result)
{
	if (!$result) {
		$result = !empty($user_data['user_type']) && $user_data['user_type'] == 'P' && ACCOUNT_TYPE == 'customer';
	}
}
