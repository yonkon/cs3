...
Registry::set('settings.store_optimization', 'dev');
...
if ($mode == 'manage') {
	...
	if (is_file($lock_file_path)) {
		...
		if (is_file($lock_file_path)) {
			fn_set_notification('W', fn_get_lang_var('warning'), str_replace('[file]', $lock_file_path, fn_get_lang_var('text_uc_unable_to_remove_upgrade_lock')));
		}
	}
	...
	if (empty($xml)) {
		...
		fn_uc_run_continuous_job('fn_uc_create_skins', array(0, DIR_UPGRADE . $_SESSION['uc_package'] . '/package', $_SESSION['uc_package'], $skip_files, $custom_skin_files, $base_ult_skin_files));
		if (PRODUCT_TYPE == 'ULTIMATE') {
			$company_ids = fn_get_all_companies_ids();
			foreach ($company_ids as $company_id) {
				fn_uc_run_continuous_job('fn_uc_create_skins', array($company_id, DIR_UPGRADE . $_SESSION['uc_package'] . '/package', $_SESSION['uc_package'], $skip_files, $custom_skin_files, $base_ult_skin_files));
			}
		}
	...
	/*fn_set_notification('W', fn_get_lang_var('warning'), 'Please refresh the page to clear browser cache if a blank page is loaded.');*/
	...
	$data .= ("\$dir_cache_templates = '" . (defined('DIR_CACHE_TEMPLATES') ? DIR_CACHE_TEMPLATES : '') . "';\n\n");
	$data .= ("\$dir_cache_registry = '" . (defined('DIR_CACHE_REGISTRY') ? DIR_CACHE_REGISTRY : '') . "';\n\n");
	$data .= ("\$dir_cache_misc = '" . (defined('DIR_CACHE_MISC') ? DIR_CACHE_MISC : '') . "';\n\n");
	...
	$product_build = (defined('PRODUCT_BUILD') && PRODUCT_BUILD) ? strtolower(PRODUCT_BUILD) : 'en';
	$dumps[$product_build] = 'db_lang_' . $product_build . '.sql';

	$add_langs = array();
	$all_sqls = fn_get_dir_contents(DIR_UPGRADE . $_SESSION['uc_package'], true, true, '.sql');
	...
	foreach ($add_langs as $add_lang) {
		if (in_array(strtoupper($add_lang), $langs)) {
			$dumps[$add_lang] = 'db_lang_' . $add_lang . '.sql';
		}
	}

	foreach ($dumps as $dump) {
		fn_uc_run_continuous_job('db_import_sql_file', array(DIR_UPGRADE . $_SESSION['uc_package'] . '/' . $dump, 16384, true, 1, true, true, true));
	...
	if (!empty($_SESSION['uc_upgrade_errors'])) {
		$view->assign('uc_upgrade_errors', true);
	...
	if (!empty($new_license)) {
		...
		$is_license_valid = true; //(isset($xml->response->license) && (string)$xml->response->license == 'VALID');
		$is_license_expired = false; //(isset($xml->response->license) && (string)$xml->response->license == 'EXPIRED');
		$is_package_available = true; //(isset($xml->response->package) && (string)$xml->response->package == 'AVAILABLE');
...
function fn_uc_check_db_errors($label, $log)
{
	...
	if (!empty($errors)) {
		foreach($errors as $error) {
			...
			fn_set_notification('E', fn_get_lang_var('error_occurred') . ': ' . $label, $error_text);
...
function fn_uc_restore_settings($new_edition = '')
{
	...
	if (!empty($was_settings_backed_up)) {
		...
		if (!empty($new_edition)) {
			CSettings::instance()->set_new_edition($new_edition);
		}
		...
		foreach ($settings as $setting) {
			CSettings::instance()->update_value($setting['name'], $setting['value'], $setting['section_name']);
		}
		...
		if (!empty($was_company_settings_backed_up)) {
			...
			foreach ($company_settings as $setting) {
				CSettings::instance()->update_value($setting['name'], $setting['value'], $setting['section_name'], false, $setting['company_id']);
			}
...
function fn_uc_save_new_license_data($new_license_data)
{
	foreach ($new_license_data as $setting_name => $setting_value) {                        
		if (CSettings::instance()->is_exists($setting_name, 'Upgrade_center')) {
			CSettings::instance()->update_value($setting_name, $setting_value, 'Upgrade_center');
		} else {
			$section_data = CSettings::instance()->get_section_by_name('Upgrade_center');
			CSettings::instance()->update(
				array(
					'name' => $setting_name,
					'value' => $setting_value,
    ...
    if (is_file($path)) {
		...
		if (file_exists($original_file)) {
			if (md5_file($original_file) != md5_file($path)) {
				...
				if (PRODUCT_TYPE == 'ULTIMATE') {
					if (preg_match('!^stores/\d+/skins/!S', $relative_file)) {
						$_relative_file = preg_replace('!^stores/\d+/skins/[\w]+/!S', 'var/skins_repository/basic/', $relative_file);
						...
						if (fn_uc_check_array_value($relative_file, $custom_skin_files) && strpos($relative_file, '/basic/') === false) {
							$_relative_file = preg_replace('!^stores/\d+/skins/([\w]+)/!S', 'var/skins_repository/${1}/', $relative_file);
						}
					}
				}
				if (strpos($relative_file, 'skins/') === 0) {
					$_relative_file = preg_replace('!skins/[\w]+/!S', 'var/skins_repository/basic/', $relative_file);

					// replace all skins except basic
					if (fn_uc_check_array_value($relative_file, $custom_skin_files) && strpos($relative_file, '/basic/') === false) {
						$_relative_file = preg_replace('!skins/([\w]+)/!S', 'var/skins_repository/${1}/', $relative_file);
					}
				}
 ...
 * @param array $custom_skin_files list of custom skin files
 * @param array $base_ult_skin_files list of files in the skins dir for the base store (ultimate edition only)
 * @return boolean true if structured created correctly, false - otherwise
 */
function fn_uc_create_skins($company_id, $path, $package, $skip_files, $custom_skin_files, $base_ult_skin_files, $log)
{
	static $skin_names_customer = array();
	static $skins_paths = array();
	static $installed_skins = array();
	
	if (empty($skin_names_customer[$company_id])) {
		if ($company_id) {
			$skin_names_customer[$company_id] = CSettings::instance()->get_value('skin_name_customer', '', $company_id);
		} else {
			$skin_names_customer[$company_id] = Registry::get('settings.skin_name_customer');
		}
	}
	
	if (empty($skins_paths[$company_id])) {
		if ($company_id) {
			$skins_paths[$company_id] = fn_get_skin_path('[relative]', '', $company_id);
		} else {
			$skins_paths[$company_id] = fn_get_skin_path('[relative]');
		}
	}
			
	if (empty($installed_skins[$company_id])) {
		$installed_skins[$company_id] = fn_get_dir_contents(DIR_ROOT . '/' . $skins_paths[$company_id], true, false);
	}
	...
	if (is_file($path)) {
		...
		if (strpos($path, DIR_REPOSITORY) !== false) {
			...
			if (strpos($path, DIR_REPOSITORY . 'customer/') !== false || strpos($path, DIR_REPOSITORY . 'mail/') !== false) {
				if (PRODUCT_TYPE != 'ULTIMATE' || PRODUCT_TYPE == 'ULTIMATE' && ($company_id || !$company_id && (strpos($path, DIR_REPOSITORY . 'mail/') !== false || strpos($path, DIR_REPOSITORY . 'customer/') !== false && fn_uc_check_array_value($path, $base_ult_skin_files)))) {
					foreach ($installed_skins[$company_id] as $s) {
						...
						if (!fn_uc_check_array_value($path, $custom_skin_files) || $s = 'basic') { // copy non-custom files only
							$files[] = str_replace(DIR_UPGRADE . $package . '/package/' . DIR_REPOSITORY, DIR_UPGRADE . $package . '/package/' . $skins_paths[$company_id] . '/' . $s . '/', $path);
						}
					}
				}
			// admin skin
			} elseif (!$company_id) {
				$files[] = str_replace(DIR_UPGRADE . $package . '/package/' . DIR_REPOSITORY, DIR_UPGRADE . $package . '/package/' . $skins_paths[$company_id] . '/' . Registry::get('settings.skin_name_admin') . '/', $path);
			}

		// Copy data from alternative skins
		} elseif (strpos($path, 'var/skins_repository/' . $skin_names_customer[$company_id]) !== false) {
			$files[] = str_replace(DIR_UPGRADE . $package . '/package/var/skins_repository/', DIR_UPGRADE . $package . '/package/' . $skins_paths[$company_id] . '/', $path);
		}
	...
	if (is_dir($path)) {
		...
		while (false !== ($entry = $dir->read())) {
			...
			fn_uc_create_skins($company_id, rtrim($path, '/') . '/' . $entry, $package, $skip_files, $custom_skin_files, $base_ult_skin_files, $log);
		}
		// Clean up
		$dir->close();
		return true;
...
function fn_uc_find_in_skins($files, $source)
{
	...
	$root_skins_path = fn_get_skin_path('[relative]');
	$skins_dirs[$root_skins_path] = fn_get_dir_contents(DIR_ROOT . '/' . $root_skins_path);
	
	if (PRODUCT_TYPE == 'ULTIMATE') {
		$company_ids = fn_get_all_companies_ids();
		foreach ($company_ids as $company_id) {
			$skins_path = fn_get_skin_path('[relative]', '', $company_id);
			$skins_dirs[$skins_path] = fn_get_dir_contents(DIR_ROOT . '/' . $skins_path);
		}
	}
	...
	foreach ($files as $file) {
		if (substr($file, 0, $len) == $base_skin) {
			foreach ($skins_dirs as $rel_path => $installed_skins) {
				foreach ($installed_skins as $skin_name) {
					$relative_name = "$rel_path/$skin_name/" . substr($file, $len);
...
function fn_uc_cleanup_cache($package, $type, $dumps = array())
{
	if ($type == 'upgrade') {
		foreach ($dumps as $dump) {
			@unlink(DIR_UPGRADE . $package . '/' . $dump . '.tmp');
		}
	} else {
...
function fn_uc_run_continuous_job($fn_name, $args)
{
	...
	if (is_file($lock_file_path)) {
		$msg = fn_get_lang_var('text_uc_another_update_process_running');
		$msg = str_replace('[filename]', $lock_file_path, $msg);
		$msg = str_replace('[url]', fn_url('upgrade_center.unlock'), $msg);
		fn_set_notification('E', fn_get_lang_var('error'), $msg);
		exit;
	} else {
		$h = fopen($lock_file_path, 'w');
		if (!$h) {
			fn_set_notification('E', fn_get_lang_var('error'), fn_get_lang_var('text_uc_cannot_lock_upgrade_process'));
			exit;
		}
		fclose($h);
	}
